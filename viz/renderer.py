...
from utils import EasyDict, open_url
...
class Renderer:
    ...
    def get_network(self, pkl, key, **tweak_kwargs):
        ...
        if data is None:
            ...
            try:
                with open_url(pkl, verbose=False) as f:
    ...
    def _get_pinned_buf(self, ref):
        ...
        if buf is None:
            buf = jt.empty(ref.shape, dtype=ref.dtype).pin_memory()
    ...
    def _apply_cmap(self, x, name='viridis'):
        ...
        hi = cmap.shape[0] - 1
        x = (x * hi + 0.5).clamp(0, hi).to(jt.int64)
        x = jt.nn.embedding(x, cmap)
        return x
    ...
    def init_network(self, res,
        ...
        **kwargs
        ...
        print(kwargs)
        G = Generator(**kwargs)
        weight_dict = jt.load(pkl)
        G.load_state_dict(weight_dict)
        
        self.G = G
        res.img_resolution = G.img_resolution
        res.num_ws = G.num_ws
        res.has_noise = any('noise_const' in name for name in G.synthesis.layer_names)
        ...
        if self.w_load is None:
            ...
            z = jt.array(np.random.RandomState(w0_seed).randn(1, 512))
        ...
        self.w_optim = jt.optim.Adam([self.w], lr=lr)
    ...
    def _render_drag_impl(self, res,
        ...
        **kwargs
        ...
        ws = jt.cat([ws[:,:6,:], self.w0[:,6:,:]], dim=1)
        ...
        if is_drag:
            X = jt.misc.linspace(0, h, h)
            Y = jt.misc.linspace(0, w, w)
            xx, yy = jt.misc.meshgrid(X, Y)
            feat_resize = jt.nn.interpolate(feat[feature_idx], [h, w], mode='bilinear')
            if self.feat_refs is None:
                self.feat0_resize = jt.nn.interpolate(feat[feature_idx].detach(), [h, w], mode='bilinear')
                self.feat_refs = []
                for point in points:
                    py, px = round(point[0]), round(point[1])
                    self.feat_refs.append(self.feat0_resize[:,:,py,px])
                # self.points0_pt = torch.Tensor(points).unsqueeze(0).to(self._device) # 1, N, 2
                self.points0_pt = jt.array(points).unsqueeze(0) # 1, N, 2
            ...
            for j, point in enumerate(points):
                ...
                direction = jt.array([targets[j][1] - point[1], targets[j][0] - point[0]])
                if jt.norm(direction) > max(2 / 512 * h, 2):
                    res.stop = False
                if jt.norm(direction) > 1:
                    distance = ((xx - point[0])**2 + (yy - point[1])**2)**0.5
                    relis, reljs = jt.where(distance < round(r1 / 512 * h))
                    direction = direction / (jt.norm(direction) + 1e-7)
                    gridh = (relis+direction[1]) / (h-1) * 2 - 1
                    gridw = (reljs+direction[0]) / (w-1) * 2 - 1
                    grid = jt.misc.stack([gridw,gridh], dim=-1).unsqueeze(0).unsqueeze(0)
                    target = jt.nn.grid_sample(feat_resize.float(), grid, align_corners=True).squeeze(2)
                    loss_motion += jt.nn.l1_loss(feat_resize[:,:,relis,reljs].detach(), target)

            loss = loss_motion
            if mask is not None:
                if mask.min() == 0 and mask.max() == 1:
                    mask_usq = mask.unsqueeze(0).unsqueeze(0)
                    loss_fix = jt.nn.l1_loss(feat_resize * mask_usq, self.feat0_resize * mask_usq)
        ...
        img = (img * 127.5 + 128).clamp(0, 255).to(jt.uint8).permute(1, 2, 0)
        if to_pil:
