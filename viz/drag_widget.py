...
class DragWidget:
    ...
    def init_mask(self, w, h):
        ...
        self.mask = jt.ones([h, w])

    def draw_mask(self, x, y):
        X = jt.misc.linspace(0, self.width, self.width)
        Y = jt.misc.linspace(0, self.height, self.height)
        yy, xx = jt.misc.meshgrid(Y, X)
        ...
        if show:
            with imgui_utils.grayed_out(self.disabled_time != 0):
                ...
                if imgui_utils.button('Reset mask', width=viz.button_w, enabled='image' in viz.result):
                    self.mask = jt.ones([self.height, self.width])
                imgui.same_line()
