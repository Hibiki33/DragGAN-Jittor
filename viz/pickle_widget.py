...
from utils import EasyDict, is_url
import imgui
import numpy as np
...
class PickleWidget:
    def __init__(self, viz):
        ...
        self.user_pkl       = ''
        self.recent_pkls    = []
        self.browse_cache   = dict() # {tuple(path, ...): [EasyDict(), ...], ...}
            ...
            if os.path.isdir(parent):
                for entry in os.scandir(parent):
                    ...
                    if entry.is_file() and pkl_regex.fullmatch(entry.name):
                        items.append(EasyDict(type='pkl', name=entry.name, path=os.path.join(parent, entry.name)))
