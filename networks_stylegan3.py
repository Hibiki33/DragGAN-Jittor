...
class SynthesisInput(jt.nn.Module):
    ...
    def execute(self, w):
        ...
        transforms = jt.array(self.transform.numpy()).unsqueeze(0) # [batch, row, col]
        freqs = jt.array(self.freqs.numpy()).unsqueeze(0) # [batch, channel, xy]
        phases = jt.array(self.phases.numpy()).unsqueeze(0) # [batch, channel]
